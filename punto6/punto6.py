...
# Load original image
img_original =  cv2.imread('/home/ieguaras/vision/punto6/4900295967065091708.jpg')
...
def trans_matrix():
    print('Valores para la transformacion similaridad.')
    x_tras = int(input('Traslacion en x: '))
    y_tras = int(input('Traslacion en y:'))
    r_angle = float(input('Angulo de rotacion: '))
    s = float(input('Valor de escalado:'))
    angle_rad = r_angle*np.pi/180

    trans_m = [
        [s*np.cos(angle_rad), s*np.sin(angle_rad), x_tras],
        [-s*np.sin(angle_rad), s*np.cos(angle_rad), y_tras]
    ]
